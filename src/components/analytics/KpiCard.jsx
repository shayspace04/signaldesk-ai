...
export default function KpiCard({ icon: Icon, color, value, label, loading, trend, sparklineData, onClick }) {
  ...
  const trendColor = trendUp ? "text-emerald-500" : trendDown ? "text-red-500" : "text-zinc-400";

  return (
    <button onClick={onClick} className="relative w-full text-left rounded-xl border border-border dark:border-[#2A2A2E] bg-white dark:bg-[#18181B] p-5 shadow-sm transition-all duration-200 hover:shadow-card hover:border-zinc-300 dark:hover:border-[#2A2A2E] cursor-pointer">
      ...
      <div className="flex items-end justify-between gap-2">
        <div>
          <p className="text-[28px] font-bold tracking-tight text-zinc-900 dark:text-[#FAFAFA] leading-none">
            {loading ? <div className="h-7 w-12 animate-pulse rounded bg-zinc-100" /> : <CountUp end={value} duration={1.5} />}
          </p>
          <p className="text-sm text-muted dark:text-[#A1A1AA] mt-1">{label}</p>
        </div>
