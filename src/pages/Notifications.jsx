...
export default function Notifications() {
  ...
  return (
    <motion.div className="flex flex-col min-h-full" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.25 }}>
      <div className="mb-6">
        <h1 className="text-[36px] font-bold tracking-tight text-primary">Notifications</h1>
        <p className="mt-1 text-sm text-muted dark:text-muted-dark">{unread.length} unread of {visible.length} total</p>
      </div>
      ...
      <div className="mb-4 flex items-center gap-3 flex-wrap">
        <div className="relative flex-1 min-w-[200px] max-w-xs">
          ...
          <input
            ...
            className="w-full rounded-xl border border-border dark:border-border-dark bg-card py-2 pl-9 pr-3 text-sm outline-none transition-all focus:border-zinc-300 dark:focus:border-border-dark focus:ring-1 focus:ring-zinc-200 dark:focus:ring-zinc-600 placeholder:text-muted dark:placeholder:text-muted-dark text-primary"
          />
        </div>
        <div className="flex rounded-xl border border-border dark:border-border-dark overflow-hidden">
          <button onClick={() => setFilter("all")}
            className={`px-3 py-2 text-xs font-medium transition-colors ${filter === "all" ? "bg-zinc-900 text-white" : "text-muted-base hover:bg-zinc-50 dark:hover:bg-[#202024]"}`}>All</button>
          <div className="w-px bg-border dark:bg-border-dark" />
          <button onClick={() => setFilter("unread")}
            className={`px-3 py-2 text-xs font-medium transition-colors ${filter === "unread" ? "bg-zinc-900 text-white" : "text-muted-base hover:bg-zinc-50 dark:hover:bg-[#202024]"}`}>Unread</button>
        </div>
        {unread.length > 0 && (
          <button onClick={handleMarkAllRead} className="rounded-xl border border-border dark:border-border-dark px-3 py-2 text-xs font-medium text-accent hover:bg-zinc-50 dark:hover:bg-[#202024] transition-colors">
            Mark all read
          </button>
        )}
        {hasReadEntries && (
          <button onClick={handleClearRead} className="rounded-xl border border-border dark:border-border-dark px-3 py-2 text-xs font-medium text-muted-base hover:bg-zinc-50 dark:hover:bg-[#202024] transition-colors">
            Clear read
          </button>
        )}
      </div>
      ...
      {m.loading ? (
        <div className="space-y-2">{[1, 2, 3, 4, 5].map((i) => <div key={i} className="h-20 animate-pulse rounded-xl bg-muted-surface" />)}</div>
      ) : filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-border dark:border-border-dark bg-muted-surface/50 dark:bg-[#202024]/50 py-20">
          <Bell size={36} className="mb-4 text-muted dark:text-muted-dark" />
          <p className="text-base font-medium text-secondary-body">No notifications</p>
          <p className="mt-1 text-sm text-muted dark:text-muted-dark">{searchQuery ? "Try a different search." : "You're all caught up!"}</p>
        </div>
      ) : (
            ...
            return (
              <div
                ...
                className={`flex items-start gap-4 rounded-xl border px-4 py-3.5 transition-all duration-200 cursor-pointer ${
                  isRead
                    ? "border-border dark:border-border-dark bg-card hover:bg-zinc-50 dark:hover:bg-[#202024]"
                    : "border-border dark:border-border-dark bg-muted-surface/50 dark:bg-[#202024]/50 hover:bg-zinc-100/50 dark:hover:bg-[#27272A]/50"
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-2">
                    <h3 className={`text-sm truncate ${isRead ? "text-secondary-body" : "font-medium text-primary"}`}>
                      {meta.label}
                    </h3>
                    <span className="flex-shrink-0 text-[10px] text-muted dark:text-muted-dark">
                      {item.created_at ? formatDistanceToNow(new Date(item.created_at), { addSuffix: true }) : ""}
                    </span>
                  </div>
                  <p className="mt-0.5 text-xs text-muted dark:text-muted-dark truncate">
                ...
                <button
                  ...
                  className="rounded-lg p-1.5 text-muted dark:text-muted-dark hover:bg-zinc-100 dark:hover:bg-[#202024] hover:text-accent transition-colors flex-shrink-0"
