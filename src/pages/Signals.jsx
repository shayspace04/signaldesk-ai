...
const TAG_STYLES = {
  billing: "bg-green-50 text-green-700", refund: "bg-purple-50 text-purple-700", urgent: "bg-orange-50 text-orange-700",
  technical: "bg-blue-50 text-blue-700", account: "bg-pink-50 text-pink-700", security: "bg-red-50 text-red-700",
  streaming: "bg-cyan-50 text-cyan-700", content: "bg-amber-50 text-amber-700", feature: "bg-indigo-50 text-indigo-700",
  data: "bg-teal-50 text-teal-700", reporting: "bg-sky-50 text-sky-700", delivery: "bg-orange-50 text-orange-700",
  merchant: "bg-yellow-50 text-yellow-700", product: "bg-fuchsia-50 text-fuchsia-700", order: "bg-lime-50 text-lime-700",
  consultation: "bg-rose-50 text-rose-700",
};

const PRIORITY_STYLES = {
  urgent: "bg-red-50 text-red-700", high: "bg-orange-50 text-orange-700",
  normal: "bg-blue-50 text-blue-700", low: "bg-zinc-100 text-zinc-500 dark:bg-[#202024] dark:text-[#A1A1AA]",
};

const COLUMNS = [
  { id: "pending", label: "New", status: "pending", color: "bg-zinc-900" },
  { id: "under_review", label: "In Review", status: "under_review", color: "bg-amber-500" },
  { id: "approved", label: "Approved", status: "approved", color: "bg-emerald-500" },
  { id: "incident_created", label: "Incident Created", status: "incident_created", color: "bg-red-500" },
];
...
function getTagStyle(category) {
  return TAG_STYLES[(category || "").toLowerCase()] || "bg-zinc-100 text-zinc-600 dark:bg-[#202024] dark:text-[#A1A1AA]";
}
...
function KanbanCard({ signal, index: cardIndex, isManager, onOpenHandoff, onDragStart, knownKnowledge, onViewKnowledge }) {
  ...
  const progress = useMemo(() => {
    if (signal.status === "incident_created") return 100;
    if (signal.status === "approved") return 80;
    if (signal.status === "under_review") return 50;
  ...
  return (
    <motion.div layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
      ...
      className="group rounded-xl border border-border dark:border-[#2A2A2E] bg-white dark:bg-[#18181B] p-4 transition-all duration-200 hover:border-zinc-300 hover:shadow-card cursor-grab active:cursor-grabbing">
      <div className="mb-3 flex items-start justify-between gap-2">
        ...
        <button className="flex-shrink-0 rounded p-1 text-muted opacity-0 transition-all hover:bg-zinc-100 hover:text-zinc-600 group-hover:opacity-100"><MoreHorizontal size={14} /></button>
      </div>
      <h3 className="mb-2.5 text-sm font-semibold leading-snug text-zinc-900 line-clamp-2">{signal.name || signal.summary || signal.id}</h3>
      {signal.detected_at && (
      ...
      <div className="mb-3">
        <div className="flex items-center justify-between mb-1">
          <span className="text-[10px] font-medium text-muted">{progress}%</span>
        </div>
      ...
      {knownKnowledge ? (
        ...
        <p className="mb-2 flex items-center gap-1.5 text-[10px] text-zinc-400 dark:text-[#71717A] px-1">
      ...
      {isManager && signal.status === "approved" && (
        <button onClick={(e) => { e.stopPropagation(); onOpenHandoff?.(signal); }}
          className="mb-3 flex w-full items-center justify-center gap-2 rounded-xl border border-border bg-white py-2 text-xs font-medium text-zinc-600 transition hover:border-zinc-300 hover:bg-zinc-50">
      ...
      <div className="flex items-center justify-between">
        <div className="flex -space-x-1.5">
          {avatars.map((a, i) => (
            <div key={i} className="flex h-6 w-6 items-center justify-center rounded-full border-2 border-white bg-zinc-200 text-[8px] font-semibold text-zinc-600">{a}</div>
          ))}
        </div>
        <div className="flex items-center gap-3">
          {attachmentCount > 0 && <span className="flex items-center gap-1 text-[11px] text-muted"><Paperclip size={12} />{attachmentCount}</span>}
          {commentCount > 0 && <span className="flex items-center gap-1 text-[11px] text-muted"><MessageSquare size={12} />{commentCount}</span>}
        </div>
      </div>
    </motion.div>
  );
...
Generated by SignalDesk · Engineering Handoff Package`, [handoffData]);
  ...
  const handleCreateIncident = async () => {
    ...
    try {
      ...
      if (incId) {
        await client.records.update("signals", signal.id, { status: "incident_created" });
        if (incId !== output) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl border border-border bg-white p-6 shadow-modal" onClick={(e) => e.stopPropagation()}>
        <div className="mb-6 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <ShieldAlert size={18} className="text-indigo-500" />
            <h2 className="text-xl font-bold text-zinc-900">Engineering Handoff</h2>
          </div>
          <button onClick={onClose} className="rounded-lg p-1.5 text-muted hover:text-zinc-700 hover:bg-zinc-100 transition-all"><X size={20} /></button>
        </div>

        <div className="mb-4 grid grid-cols-2 gap-3">
          <div className="rounded-xl bg-zinc-50 p-3"><p className="text-xs text-muted">Summary</p><p className="mt-1 text-sm font-medium text-zinc-900">{handoffData.summary || "N/A"}</p></div>
          <div className="rounded-xl bg-zinc-50 p-3"><p className="text-xs text-muted">Root Cause</p><p className="mt-1 text-sm font-medium text-zinc-900">{handoffData.rootCause || "N/A"}</p></div>
          <div className="rounded-xl bg-zinc-50 p-3"><p className="text-xs text-muted">Priority</p><p className="mt-1 text-sm font-medium text-zinc-900">{handoffData.priority}</p></div>
          <div className="rounded-xl bg-zinc-50 p-3"><p className="text-xs text-muted">Confidence</p><p className="mt-1 text-sm font-medium text-zinc-900">{handoffData.confidence}%</p></div>
          <div className="rounded-xl bg-zinc-50 p-3"><p className="text-xs text-muted">Affected Customers</p><p className="mt-1 text-sm font-medium text-zinc-900">{handoffData.affectedCustomers}</p></div>
          <div className="rounded-xl bg-zinc-50 p-3"><p className="text-xs text-muted">Affected Tickets</p><p className="mt-1 text-sm font-medium text-zinc-900">{handoffData.affectedTickets}</p></div>
        </div>
        ...
        {Array.isArray(handoffData.evidence) && handoffData.evidence.length > 0 && (
          <div className="mb-4 rounded-xl bg-zinc-50 p-3">
            ...
            {handoffData.evidence.map((e, i) => <p key={i} className="text-sm text-zinc-700">• {e}</p>)}
          </div>
        )}
        ...
        {handoffData.timeline && (
          <div className="mb-4 rounded-xl bg-zinc-50 p-3">
            ...
            <p className="mt-1 text-sm text-zinc-700">Detected: {new Date(handoffData.timeline).toLocaleDateString("en-US", { day: "numeric", month: "short", year: "numeric" })}</p>
          </div>
        )}

        <div className="mb-4 rounded-xl border border-border bg-zinc-50 p-4">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2"><FileText size={15} className="text-zinc-500" /><span className="text-xs font-semibold text-zinc-500 uppercase tracking-wide">Markdown Preview</span></div>
            <button onClick={() => handleCopy(markdown, "Markdown")} className="flex items-center gap-1.5 rounded-xl border border-border bg-white px-3 py-1.5 text-xs font-medium text-zinc-500 hover:text-zinc-700 transition-colors">
              {copied === "Markdown" ? <><Check size={13} className="text-green-500" /> Copied</> : <><Copy size={13} /> Copy</>}
            </button>
          </div>
          <pre className="overflow-x-auto rounded-xl bg-white p-3 text-xs leading-relaxed text-zinc-700 whitespace-pre-wrap font-mono max-h-48">{markdown}</pre>
        </div>

        <div className="mb-5 rounded-xl border border-border bg-zinc-50 p-4">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2"><Code size={15} className="text-zinc-500" /><span className="text-xs font-semibold text-zinc-500 uppercase tracking-wide">JSON Preview</span></div>
            <button onClick={() => handleCopy(json, "JSON")} className="flex items-center gap-1.5 rounded-xl border border-border bg-white px-3 py-1.5 text-xs font-medium text-zinc-500 hover:text-zinc-700 transition-colors">
              {copied === "JSON" ? <><Check size={13} className="text-green-500" /> Copied</> : <><Copy size={13} /> Copy</>}
            </button>
          </div>
          <pre className="overflow-x-auto rounded-xl bg-white p-3 text-xs leading-relaxed text-zinc-700 whitespace-pre-wrap font-mono max-h-48">{json}</pre>
        </div>
        ...
        <div className="flex gap-3">
          ...
          <button onClick={onClose} className="flex-1 rounded-xl border border-border py-3 text-sm font-medium text-zinc-600 hover:bg-zinc-50 transition-all">Cancel</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-3 rounded-xl border border-border bg-surface p-4">
      <input placeholder="Signal name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
        className="w-full rounded-xl border border-border bg-white p-2.5 text-sm text-zinc-900 outline-none transition-all focus:border-zinc-300 focus:ring-1 focus:ring-zinc-200" />
      <textarea placeholder="Summary" rows={3} value={form.summary} onChange={(e) => setForm({ ...form, summary: e.target.value })}
        className="w-full rounded-xl border border-border bg-white p-2.5 text-sm text-zinc-900 outline-none transition-all focus:border-zinc-300 focus:ring-1 focus:ring-zinc-200 resize-none" />
      <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })}
        className="w-full rounded-xl border border-border bg-white p-2.5 text-sm text-zinc-900 outline-none transition-all focus:border-zinc-300">
      ...
      <div className="flex gap-2">
        ...
        <button onClick={onClose} className="rounded-xl border border-border px-4 py-2.5 text-sm text-zinc-500 hover:text-zinc-900 transition-all">Cancel</button>
      </div>
    </div>
  );
}
...
export default function Signals() {
  ...
  const statusMap = (s) => {
    if (s.status === "memory" || s.status === "resolved") return "resolved";
    if (s.status === "incident_created") return "incident_created";
    if (s.status === "approved") return "approved";
    if (s.status === "under_review") return "under_review";
    return "pending";
  };

  const columns = useMemo(() => COLUMNS.filter((c) => {
    if (c.id === "resolved") return false;
    return true;
  }).map((col) => ({
    ...col,
    cards: signals.filter((s) => statusMap(s) === col.status),
  })), [signals]);
  ...
  const handleDrop = useCallback(async (e, targetStatus) => {
    ...
    if (!signal || statusMap(signal) === targetStatus) return;
    const label = COLUMNS.find((c) => c.status === targetStatus)?.label || targetStatus;
    try {
      await client.records.update("signals", signalId, { status: targetStatus });
      toast.success(`Signal moved to ${label}`);
      refresh();
    } catch (err) {
      toast.error(err?.message || "Failed to update signal status");
    }
    setDragId(null);
  }, [signals, refresh]);
  ...
  return (
    <motion.div className="flex flex-col min-h-full" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.25 }}>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-[36px] font-bold tracking-tight text-zinc-900">Signals</h1>
          <p className="mt-1 text-sm text-muted">{signals.length} signal{signals.length !== 1 ? "s" : ""} detected</p>
        </div>
        <div className="flex items-center rounded-xl border border-border overflow-hidden shadow-sm">
          <button onClick={() => setView("kanban")} className={`flex items-center gap-1.5 px-3.5 py-2 text-xs font-medium transition-all duration-150 ${view === "kanban" ? "bg-zinc-900 text-white" : "text-zinc-500 hover:bg-zinc-50"}`}>
            <Columns3 size={14} /> Kanban
          </button>
          <div className="w-px h-4 bg-border" />
          <button onClick={() => setView("list")} className={`flex items-center gap-1.5 px-3.5 py-2 text-xs font-medium transition-all duration-150 ${view === "list" ? "bg-zinc-900 text-white" : "text-zinc-500 hover:bg-zinc-50"}`}>
            <List size={14} /> List
          </button>
        </div>
      </div>
      ...
      {loading ? (
        <div className="flex gap-6 flex-1">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="flex-1 space-y-3"><div className="h-6 w-24 animate-pulse rounded bg-zinc-100" />{[1, 2, 3].map((j) => <div key={j} className="h-40 animate-pulse rounded-xl bg-zinc-100" />)}</div>
          ))}
        </div>
      ) : view === "list" ? (
        <div className="space-y-2">
          {signals.map((s, i) => (
            <div key={s.id} className="flex items-center justify-between rounded-xl border border-border bg-white p-4 shadow-sm transition-all hover:border-zinc-300 hover:shadow-card">
              <div className="min-w-0 flex-1">
                <p className="text-sm font-medium text-zinc-900 truncate">{s.name || s.summary || s.id}</p>
            ...
            <div key={col.id} className="flex-1 min-w-[280px] flex flex-col">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-medium text-muted bg-zinc-100 rounded-full px-2 py-0.5">{col.cards.length}</span>
                </div>
                <button className="text-muted hover:text-zinc-700 p-1 rounded hover:bg-zinc-100 transition-all"><MoreHorizontal size={14} /></button>
              </div>
              <div
                ...
                {col.cards.length === 0 ? (
                  <div className="flex items-center justify-center h-32 rounded-xl border border-dashed border-zinc-200 bg-zinc-50/50"><p className="text-xs text-muted">No signals</p></div>
                ) : (
              ...
              <button onClick={() => setCreatingIn(creatingIn === col.id ? null : col.id)}
                className="flex items-center gap-1.5 mt-3 px-3 py-2 rounded-xl text-xs font-medium text-muted hover:text-zinc-700 hover:bg-zinc-50 border border-transparent hover:border-border transition-all">
