...
function Section({ title, icon: Icon, children }) {
  return (
    <div className="rounded-xl border border-border dark:border-border-dark bg-card p-6 shadow-sm">
      <div className="flex items-center gap-3 mb-5">
        {Icon && <Icon size={18} className="text-muted-base" />}
...
function CollapsibleSection({ title, icon: Icon, defaultOpen, badge, children }) {
  ...
  return (
    <div className="rounded-xl border border-border dark:border-border-dark bg-card shadow-sm overflow-hidden">
      <button onClick={() => setOpen(!open)}
        className="flex w-full items-center justify-between px-6 py-4 hover:bg-zinc-50 dark:hover:bg-muted-surface transition-colors">
        <div className="flex items-center gap-3">
          {Icon && <Icon size={18} className="text-muted-base" />}
          <span className="text-sm font-semibold text-primary">{title}</span>
          {badge != null && (
            <span className="rounded-md bg-zinc-100 dark:bg-zinc-800 px-2 py-0.5 text-[11px] font-medium text-muted-base">{badge}</span>
          )}
        </div>
...
function InfoRow({ label, value, children }) {
  return (
    <div className="flex items-center justify-between py-2.5 border-b border-border dark:border-border-dark last:border-0">
      <span className="text-sm text-muted dark:text-muted-dark">{label}</span>
      {children || <span className="text-sm text-primary font-mono truncate max-w-[300px] ml-4">{value ?? "-"}</span>}
    </div>
  );
}
  ...
  return (
    <div className="flex items-center justify-between py-2.5 border-b border-border dark:border-border-dark last:border-0">
      <div className="min-w-0 flex-1 pr-4">
        ...
        {description && <p className="text-xs text-muted dark:text-muted-dark mt-0.5">{description}</p>}
      </div>
      <button
        disabled={disabled}
        ...
        className={`relative inline-flex h-6 w-10 shrink-0 cursor-pointer items-center rounded-full transition-colors duration-200 ${
          checked ? "bg-zinc-900 dark:bg-zinc-700" : "bg-zinc-200 dark:bg-zinc-700"
...
function SliderControl({ label, value, min, max, step, suffix, onChange }) {
  ...
  return (
    <div className="py-2.5 border-b border-border dark:border-border-dark last:border-0">
      <div className="flex items-center justify-between mb-2">
        <span className="text-sm font-medium text-primary">{label}</span>
        <span className="text-sm font-mono text-muted-base tabular-nums">{value}{suffix || ""}</span>
      </div>
      <div className="relative h-2">
        <div className="absolute inset-0 rounded-full bg-muted-surface" />
    ...
    <div className="flex items-center justify-between py-2.5 border-b border-border dark:border-border-dark last:border-0">
      ...
      <div className="flex items-center gap-2">
        <input type="number" min={min} max={max} value={value}
          ...
          className="w-16 rounded-lg border border-border dark:border-border-dark bg-card px-2.5 py-1.5 text-sm font-mono text-primary text-center outline-none focus:border-zinc-300 dark:focus:border-zinc-600" />
  ...
  return (
    <div className="flex items-center justify-between py-2.5 border-b border-border dark:border-border-dark last:border-0">
      ...
      <select value={value} onChange={(e) => onChange(e.target.value)}
        className="rounded-lg border border-border dark:border-border-dark bg-card px-3 py-1.5 text-sm text-primary outline-none focus:border-zinc-300 dark:focus:border-zinc-600">
        {options.map((o) => (
...
function StatusDot({ status }) {
  const colors = { connected: "bg-emerald-500", disconnected: "bg-red-500", checking: "bg-amber-400 animate-pulse", unknown: "bg-zinc-300 dark:bg-zinc-600" };
  ...
  return (
    <div className="rounded-xl border border-border dark:border-border-dark bg-card p-4">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          <div className="rounded-lg bg-muted-surface p-2.5">
            {Icon && <Icon size={20} className="text-muted-base" />}
          </div>
          <div>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className={`text-xs font-medium ${
            ...
            status === "disconnected" ? "text-red-600 dark:text-red-400" : "text-muted-base"
          }`}>
      ...
      <div className="mt-3 flex gap-2">
        <button onClick={onTest} disabled={testing || status === "checking"}
          className="flex items-center gap-1.5 rounded-lg border border-border dark:border-border-dark px-3 py-1.5 text-xs font-medium text-secondary-body hover:bg-zinc-50 dark:hover:bg-zinc-700 transition-colors disabled:opacity-50">
...
function WorkflowStep({ icon: Icon, label, value, active }) {
  return (
    <div className={`flex items-center gap-3 px-4 py-2.5 rounded-lg transition-colors ${active ? "bg-muted-surface" : ""}`}>
      {Icon && <Icon size={16} className={active ? "text-primary" : "text-zinc-400 dark:text-zinc-500"} />}
      <span className={`text-sm flex-1 ${active ? "text-primary font-medium" : "text-muted-base"}`}>{label}</span>
      {value && <span className="text-xs font-mono text-muted-base bg-muted-surface rounded-md px-2 py-0.5">{value}</span>}
    </div>
  );
}
...
function WorkflowPreview({ config }) {
  ...
  return (
    <div className="rounded-xl border border-border dark:border-border-dark bg-zinc-50/50 dark:bg-muted-surface/50 p-5">
      <div className="flex items-center gap-2 mb-4">
        <Activity size={16} className="text-muted-base" />
        <h3 className="text-sm font-semibold text-primary">Current AI Workflow</h3>
        <span className="text-[10px] font-medium text-zinc-400 dark:text-zinc-500 ml-auto">{activeCount} steps active</span>
      </div>
...
function ConfigSummary({ config }) {
  ...
  return (
    <div className="rounded-xl border border-border dark:border-border-dark bg-card p-5">
      <div className="flex items-center gap-2 mb-4">
        <BarChart3 size={16} className="text-muted-base" />
        <h3 className="text-sm font-semibold text-primary">Current Policy</h3>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
        {items.map((item) => (
          <div key={item.label} className="rounded-lg bg-zinc-50 dark:bg-muted-surface p-3">
            <p className="text-[10px] font-medium text-muted dark:text-muted-dark uppercase tracking-wide">{item.label}</p>
...
function SimulationPanel({ config }) {
  ...
  return (
    <div className="rounded-xl border border-border dark:border-border-dark bg-card p-5">
      <div className="flex items-center gap-2 mb-4">
        <Play size={16} className="text-muted-base" />
        <h3 className="text-sm font-semibold text-primary">AI Detection Simulator</h3>
        <span className="text-[10px] text-muted dark:text-muted-dark ml-auto">Preview only · No records created</span>
      </div>
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-3">
          <input placeholder="Sample Ticket Title" value={form.title}
            onChange={(e) => setForm({ ...form, title: e.target.value })}
            className="w-full rounded-lg border border-border dark:border-border-dark bg-card px-3 py-2 text-sm text-primary outline-none focus:border-zinc-300 dark:focus:border-zinc-600 placeholder:text-zinc-400 dark:placeholder:text-zinc-500" />
          <textarea placeholder="Sample Description" rows={2} value={form.description}
            onChange={(e) => setForm({ ...form, description: e.target.value })}
            className="w-full rounded-lg border border-border dark:border-border-dark bg-card px-3 py-2 text-sm text-primary outline-none focus:border-zinc-300 dark:focus:border-zinc-600 placeholder:text-zinc-400 dark:placeholder:text-zinc-500 resize-none" />
          <div className="grid grid-cols-2 gap-2">
            <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })}
              className="rounded-lg border border-border dark:border-border-dark bg-card px-3 py-2 text-sm text-primary outline-none">
            ...
            <input type="number" min={1} max={20} value={form.numSimilar}
              ...
              className="rounded-lg border border-border dark:border-border-dark bg-card px-3 py-2 text-sm text-primary outline-none focus:border-zinc-300 text-center" placeholder="Similar tickets" />
          </div>
        ...
        <div className="rounded-lg border border-border dark:border-border-dark bg-zinc-50/50 dark:bg-muted-surface/50 p-4">
          {!result ? (
            <div className="flex items-center justify-center h-full text-sm text-muted dark:text-muted-dark">
              <Eye size={32} className="opacity-20 mr-2" /> Enter details and click Simulate
            </div>
          ) : (
            <div className="space-y-2.5">
              <div className="flex items-center justify-between">
                <span className="text-xs text-muted dark:text-muted-dark">Similarity</span>
                <span className={`text-sm font-bold tabular-nums ${result.similarity >= config.similarityThreshold ? "text-emerald-600 dark:text-emerald-400" : "text-muted-base"}`}>{result.similarity}%</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs text-muted dark:text-muted-dark">Confidence</span>
                <span className="text-sm font-bold tabular-nums text-primary">{result.confidence}%</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs text-muted dark:text-muted-dark">Risk Score</span>
                <span className={`text-sm font-bold tabular-nums ${result.risk >= config.incidentEscalationThreshold / 10 ? "text-red-600 dark:text-red-400" : "text-muted-base"}`}>{result.risk}/10</span>
              </div>
...
function SimBadge({ label, active, className }) {
  return (
    <div className={`flex items-center gap-1.5 rounded-md px-2.5 py-1.5 text-xs font-medium ${active ? "bg-emerald-50 dark:bg-emerald-950/20 text-emerald-700 dark:text-emerald-300" : "bg-muted-surface text-zinc-400 dark:text-zinc-500"} ${className || ""}`}>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        <div className="rounded-xl border border-border dark:border-border-dark bg-card p-4">
          <div className="flex items-center gap-2 mb-3"><Settings2 size={15} className="text-muted-base" /><h3 className="text-sm font-semibold text-primary">Detection Tuning</h3></div>
        ...
        <div className="rounded-xl border border-border dark:border-border-dark bg-card p-4">
          <div className="flex items-center gap-2 mb-3"><Users size={15} className="text-muted-base" /><h3 className="text-sm font-semibold text-primary">Impact & Risk</h3></div>
        ...
        <div className="rounded-xl border border-border dark:border-border-dark bg-card p-4">
          <div className="flex items-center gap-2 mb-3"><Zap size={15} className="text-muted-base" /><h3 className="text-sm font-semibold text-primary">Mode Preview</h3></div>
          <div className="space-y-2 text-xs text-muted dark:text-muted-dark">
            <p><span className="font-medium text-primary">Effective Min Similarity:</span> {Math.round(config.similarityThreshold * modeMultiplier.sim)}%</p>
            <p><span className="font-medium text-primary">Effective Min Tickets:</span> {Math.max(1, Math.round(config.minSimilarTickets * modeMultiplier.min))}</p>
            <p><span className="font-medium text-primary">Effective Risk Threshold:</span> {Math.round(config.incidentEscalationThreshold * modeMultiplier.risk)}%</p>
            <p className="pt-1 text-[10px] text-zinc-400 dark:text-zinc-500">{config.detectionMode === "conservative" ? "Requires higher confidence before acting" : config.detectionMode === "aggressive" ? "Acts on weaker signals with lower thresholds" : "Standard detection with default thresholds"}</p>
          </div>
        </div>
      </div>
      ...
      {/* Automation Toggles */}
      <div className="rounded-xl border border-border dark:border-border-dark bg-card p-4">
        <div className="flex items-center gap-2 mb-3"><Activity size={15} className="text-muted-base" /><h3 className="text-sm font-semibold text-primary">Automation</h3></div>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <div>
            <p className="text-[10px] font-semibold text-muted dark:text-muted-dark uppercase tracking-wide mb-1 flex items-center gap-1"><Radio size={11} /> Signal Automation</p>
          ...
          <div>
            <p className="text-[10px] font-semibold text-muted dark:text-muted-dark uppercase tracking-wide mb-1 flex items-center gap-1"><Shield size={11} /> Incident Automation</p>
          ...
          <div>
            <p className="text-[10px] font-semibold text-muted dark:text-muted-dark uppercase tracking-wide mb-1 flex items-center gap-1"><ExternalLink size={11} /> Engineering Automation</p>
          ...
          <div>
            <p className="text-[10px] font-semibold text-muted dark:text-muted-dark uppercase tracking-wide mb-1 flex items-center gap-1"><Mail size={11} /> Communication</p>
          ...
          <div>
            <p className="text-[10px] font-semibold text-muted dark:text-muted-dark uppercase tracking-wide mb-1 flex items-center gap-1"><BookOpen size={11} /> Knowledge</p>
      ...
      <div className="flex justify-end">
        <button onClick={resetConfig}
          className="flex items-center gap-1.5 rounded-lg border border-border dark:border-border-dark px-3 py-1.5 text-xs font-medium text-muted-base hover:bg-zinc-50 dark:hover:bg-zinc-700 transition-colors">
...
export default function Settings() {
    ...
    async function load() {
      try {
        ...
        try {
          ...
          let acct = null;
          try {
            const surface = await client.podSurfaces.get(client.podId, "gmail");
            if (surface) {
              connected = surface.is_enabled !== false;
              acct = surface.account?.email || surface.account?.name || surface.email || null;
            }
          } catch {}
          if (!connected) {
            try {
              const connList = await client.connectors.list({ limit: 50 });
              const items = connList.items || [];
              const gmail = items.find((c) => (c.name || c.id || "").toLowerCase().includes("gmail"));
              if (gmail) {
                connected = true;
                acct = gmail.name || gmail.id || "Gmail";
              }
            } catch {}
          }
        ...
        /* Linear status */
        try {
          const lr = await client.functions.run("test_linear_connector", { input: {} }).catch(() => null);
          const lResult = lr?.output_data || lr?.output || lr;
          setLinearStatus(lResult?.connected ? "connected" : "disconnected");
  ...
  const handleLemmaTest = useCallback(async () => {
    setLemmaTesting(true);
    try {
      await client.records.list("tickets", { limit: 1 });
    } catch {
      /* ping succeeded — Lemma API responded */
    } finally {
      setLemmaTesting(false);
    }
  }, []);
  ...
  const handleGmailReconnect = useCallback(() => {
    window.open("https://console.lemma.ai/pod/" + client.podId + "/surfaces", "_blank", "noopener,noreferrer");
  }, []);
  ...
  return (
    <motion.div className="flex flex-col min-h-full space-y-6 max-w-5xl" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.25 }}>
      <div>
        <h1 className="text-[36px] font-bold tracking-tight text-primary">Settings</h1>
        <p className="mt-1 text-sm text-muted dark:text-muted-dark">Configure workspace, integrations, and AI detection pipeline.</p>
      </div>
        ...
        <div className="space-y-1">
          <InfoRow label="Workspace">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-primary font-medium">{workspace.name}</span>
            </div>
          </InfoRow>
          <InfoRow label="Current User" value="Support Manager" />
          <InfoRow label="Role">
            <div className="flex gap-1.5">
              <button onClick={() => setRole("support_agent")} className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium transition-all ${role === "support_agent" ? "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 ring-2 ring-blue-500" : "bg-zinc-100 text-secondary-body dark:bg-zinc-800 dark:text-muted-base hover:bg-zinc-200 dark:hover:bg-zinc-700"}`}><UserCheck size={12} />Agent</button>
              <button onClick={() => setRole("support_manager")} className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium transition-all ${role === "support_manager" ? "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 ring-2 ring-blue-500" : "bg-zinc-100 text-secondary-body dark:bg-zinc-800 dark:text-muted-base hover:bg-zinc-200 dark:hover:bg-zinc-700"}`}><UserCog size={12} />Manager</button>
            </div>
          </InfoRow>
          <InfoRow label="Theme">
            <ThemeToggle />
      ...
      <CollapsibleSection title="Integrations" icon={Link2} defaultOpen={true}>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <ConnectorCard icon={Database} name="Lemma" description="Platform API & data layer" status="connected"
            ...
            lastSync="Real-time"
            onTest={handleLemmaTest}
            testing={lemmaTesting} />
      ...
      <CollapsibleSection title="AI Detection" icon={Radio} defaultOpen={true} badge="NEW">
        {/* Workspace tabs */}
        <div className="flex flex-wrap gap-1 mb-5 pb-3 border-b border-border dark:border-border-dark">
          {aiWorkspaces.map((w) => (
            <button key={w.id} onClick={() => setAiWorkspaceTab(w.id)}
              className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium transition-all ${
                aiWorkspaceTab === w.id
                  ? "bg-zinc-900 text-white dark:bg-zinc-700 dark:text-white"
                  : "text-muted-base hover:bg-zinc-100 dark:hover:bg-zinc-800"
              }`}>
        ...
        <CollapsibleSection title="Developer Tools" icon={Bug} defaultOpen={false}>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div className="rounded-xl border border-border dark:border-border-dark bg-card p-4 flex flex-col gap-3">
              <div>
                <p className="text-sm font-medium text-primary">Pod Info</p>
                <p className="text-xs text-muted dark:text-muted-dark">Connection details</p>
              </div>
            ...
            <div className="rounded-xl border border-border dark:border-border-dark bg-card p-4 flex flex-col gap-3">
              <div>
                <p className="text-sm font-medium text-primary">Agents ({agents.length})</p>
                <p className="text-xs text-muted dark:text-muted-dark">Deployed agents</p>
              </div>
              {loading ? <div className="h-4 w-full animate-pulse rounded bg-zinc-100" /> : agents.length === 0 ? <p className="text-xs text-muted dark:text-muted-dark">None</p> : (
                <div className="max-h-32 overflow-y-auto space-y-1">
                  {agents.map((a) => (
                    <div key={a.name || a.id} className="rounded-lg bg-zinc-50 dark:bg-muted-surface p-2"><p className="text-xs font-medium text-primary truncate">{a.name || a.id}</p></div>
                  ))}
                </div>
              )}
            </div>
            ...
            <div className="rounded-xl border border-border dark:border-border-dark bg-card p-4 flex flex-col gap-3">
              <div>
                <p className="text-sm font-medium text-primary">Functions ({funcs.length})</p>
                <p className="text-xs text-muted dark:text-muted-dark">Available functions</p>
              </div>
              {loading ? <div className="h-4 w-full animate-pulse rounded bg-zinc-100" /> : funcs.length === 0 ? <p className="text-xs text-muted dark:text-muted-dark">None</p> : (
                <div className="max-h-32 overflow-y-auto space-y-1">
                  {funcs.map((f) => (
                    <div key={f.name || f.id} className="rounded-lg bg-zinc-50 dark:bg-muted-surface p-2"><p className="text-xs font-medium text-primary truncate">{f.name || f.id}</p></div>
                  ))}
                </div>
              )}
            </div>
            ...
            <div className="rounded-xl border border-border dark:border-border-dark bg-card p-4 flex flex-col gap-3">
              <div>
                <p className="text-sm font-medium text-primary">Workflows ({workflows.length})</p>
                <p className="text-xs text-muted dark:text-muted-dark">Active workflows</p>
              </div>
              {loading ? <div className="h-4 w-full animate-pulse rounded bg-zinc-100" /> : workflows.length === 0 ? <p className="text-xs text-muted dark:text-muted-dark">None</p> : (
                <div className="max-h-32 overflow-y-auto space-y-1">
                  {workflows.map((w) => (
                    <div key={w.name || w.id} className="rounded-lg bg-zinc-50 dark:bg-muted-surface p-2"><p className="text-xs font-medium text-primary truncate">{w.name || w.id}</p></div>
                  ))}
                </div>
              )}
            </div>
            ...
            <div className="rounded-xl border border-border dark:border-border-dark bg-card p-4 flex flex-col gap-3">
              <div>
                <p className="text-sm font-medium text-primary">Tables ({tables.length})</p>
                <p className="text-xs text-muted dark:text-muted-dark">Data tables</p>
              </div>
              {loading ? <div className="h-4 w-full animate-pulse rounded bg-zinc-100" /> : tables.length === 0 ? <p className="text-xs text-muted dark:text-muted-dark">None</p> : (
                <div className="max-h-32 overflow-y-auto space-y-1">
                  {tables.map((t) => (
                    <div key={t.name || t.id} className="rounded-lg bg-zinc-50 dark:bg-muted-surface p-2"><p className="text-xs font-medium text-primary truncate">{t.name || t.id}</p><p className="text-[10px] text-muted dark:text-muted-dark">{t.columns?.length || 0} columns</p></div>
                  ))}
                </div>
              )}
            </div>
            ...
            <div className="rounded-xl border border-border dark:border-border-dark bg-card p-4 flex flex-col gap-3">
              <div>
                <p className="text-sm font-medium text-primary">Migrate Workspaces</p>
                <p className="text-xs text-muted dark:text-muted-dark">Tag records with workspaceId</p>
              </div>
            ...
            <div className="rounded-xl border border-border dark:border-border-dark bg-card p-4 flex flex-col gap-3">
              <div>
                <p className="text-sm font-medium text-primary">Validate Workspaces</p>
                <p className="text-xs text-muted dark:text-muted-dark">Per-workspace record counts</p>
              </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-xs border-collapse">
                    <thead><tr className="border-b border-border dark:border-border-dark">
                      <th className="text-left py-1 pr-2 font-medium text-muted-base">Workspace</th>
                      {Object.keys(Object.values(validateResult)[0] ?? {}).map((t) => {
                        ...
                        return <th key={t} className="text-right py-1 px-1 font-medium text-muted-base">{labels[t] || t}</th>;
                      })}
                    </tr></thead>
                    <tbody>{Object.entries(validateResult).map(([ws, counts]) => (
                      <tr key={ws} className="border-b border-border/50"><td className="py-1 pr-2 text-primary font-medium capitalize">{ws}</td>
                        {Object.values(counts).map((count, i) => <td key={i} className={`text-right py-1 px-1 ${count === 0 ? "text-red-500 dark:text-red-400 font-bold" : "text-secondary-body"}`}>{count}</td>)}
                      </tr>
                    ))}</tbody>
                  </table>
                </div>
            ...
            {/* Refresh */}
            <div className="rounded-xl border border-border dark:border-border-dark bg-card p-4 flex flex-col gap-3">
              <div><p className="text-sm font-medium text-primary">Refresh Metrics</p><p className="text-xs text-muted dark:text-muted-dark">Invalidate all cached data</p></div>
            ...
            {/* AI Detection */}
            <div className="rounded-xl border border-border dark:border-border-dark bg-card p-4 flex flex-col gap-3">
              <div><p className="text-sm font-medium text-primary">Run AI Detection</p><p className="text-xs text-muted dark:text-muted-dark">Trigger detection for all tickets in every workspace</p></div>
              ...
              {aiTestReport && (
                <div className="max-h-48 overflow-y-auto space-y-0.5">
                  {aiTestReport.steps.map((s, i) => (
                    <p key={i} className={`text-xs ${s.passed === true ? "text-emerald-600 dark:text-emerald-400" : s.passed === false ? "text-red-600 dark:text-red-400" : s.passed === "running" ? "text-blue-600 dark:text-blue-400" : "text-amber-600 dark:text-amber-400"}`}>
                      {s.passed === true ? "\u2713" : s.passed === false ? "\u2717" : s.passed === "running" ? "\u2192" : "!"} {s.name}
                      {s.detail ? <span className="text-zinc-400 dark:text-zinc-500 ml-1">\u2014 {s.detail}</span> : ""}
                    </p>
                  ))}
                </div>
              )}
            ...
            {/* Export */}
            <div className="rounded-xl border border-border dark:border-border-dark bg-card p-4 flex flex-col gap-3">
              <div><p className="text-sm font-medium text-primary">Export Diagnostics</p><p className="text-xs text-muted dark:text-muted-dark">Download JSON report</p></div>
            ...
            {/* Deploy Sync */}
            <div className="rounded-xl border border-border dark:border-border-dark bg-card p-4 flex flex-col gap-3">
              <div><p className="text-sm font-medium text-primary">Deploy Sync</p><p className="text-xs text-muted dark:text-muted-dark">Check pod resource sync status</p></div>
              ...
              {deploySummary && deploySummary.loading !== true && (
                <div className="text-xs space-y-0.5">
                  <p className="text-secondary-body">Tables: {deploySummary.tables?.total || "?"}</p>
                  <p className="text-secondary-body">Functions: {deploySummary.functions || "?"}</p>
                  <p className="text-secondary-body">Agents: {deploySummary.agents || "?"}</p>
                  <p className="text-secondary-body">Workflows: {deploySummary.workflows || "?"}</p>
                  ...
                  {deploySummary.warnings?.length > 0 && deploySummary.warnings.map((w, i) => (
                    <p key={i} className="text-amber-600 dark:text-amber-400"><AlertTriangle size={10} className="inline mr-1" />{w}</p>
                  ))}
                  <p className="text-[10px] text-zinc-400 dark:text-zinc-500 mt-1">Deploy via: <code className="bg-zinc-100 dark:bg-zinc-800 px-1 rounded">.\deploy-sync.ps1 -Command sync</code></p>
                </div>
              )}
            </div>
