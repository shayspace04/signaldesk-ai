...
async def resolve_ticket(ctx: FunctionContext, data: ResolveTicketInput) -> ResolveTicketOutput:
    ...
    if not ticket_id:
        raise RuntimeError(f"draft {data.draft_id} has no ticket_id")
    ...
    _audit(pod, "draft.approved", actor_type="user", actor_user_id=actor,
           resource_type="draft", resource_id=data.draft_id, ticket_id=str(ticket_id),
           details={"reviewer_notes": data.reviewer_notes, "approval_id": appr["id"]})
    return ResolveTicketOutput(ticket_id=str(ticket_id), draft_id=data.draft_id, approval_id=appr["id"])
